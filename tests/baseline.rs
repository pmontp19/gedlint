...
use gedlint::{apply_baseline, baseline_from_report, baseline_to_json, parse_baseline, Baseline, BaselineEntry, Category, Diag, Report, Severity, Version};
...
fn write_baseline_exits_0_and_records_every_finding() {
    ...
    let (code, out, _) = run(&[f.to_str().unwrap(), "--write-baseline", base.to_str().unwrap(), "--no-color"]);
    assert_eq!(code, 0, "recording a baseline must not fail CI bootstrap: {}", out);
    ...
    for code in ["E201", "W305", "U502"] {
        assert!(b.entries.iter().any(|e| e.code == code), "{} missing: {:?}", code, b.entries);
    }
    // Entries carry counts, not line numbers.
    assert!(b.entries.iter().all(|e| e.count == 1 && !e.fingerprint.contains("line")), "{:?}", b.entries);
}
...
fn baseline_makes_the_recorded_tree_pass() {
    ...
    let _ = run(&[f.to_str().unwrap(), "--write-baseline", base.to_str().unwrap()]);
    let (code, out, _) = run(&[f.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--no-color"]);
...
fn new_finding_of_a_new_shape_still_fails() {
    ...
    let f2 = write(&d, "t2.ged", &messy_plus("0 @I9@ INDI\n1 NAME Nou /P/\n1 FAMS @F7@\n"));
    let (code, out, _) = run(&[f2.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--no-color"]);
    assert_eq!(code, 2, "a new error must fail: {}", out);
    assert!(out.contains("1 new diagnostics"), "{}", out);
    assert!(out.contains("3 baselined"), "{}", out);
    assert!(out.contains("FAMS @F7@") || out.contains("E201"), "the new finding is listed: {}", out);
}
...
fn count_exhaustion_reports_surplus() {
    ...
    // code-based muting.
    let f2 = write(&d, "t2.ged", &messy_plus("0 @I8@ INDI\n1 NAME Nou /P/\n1 FAMC @F9@\n"));
    let (code, out, _) = run(&[f2.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--no-color"]);
...
fn inserting_10_lines_does_not_invalidate_the_baseline() {
    ...
    let (code, out, _) = run(&[f2.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--no-color"]);
    assert_eq!(code, 0, "a line insertion must not invalidate the baseline: {}", out);
    assert!(out.contains("0 new diagnostics"), "{}", out);
    assert!(out.contains("3 baselined"), "{}", out);
}
...
fn resolved_findings_are_reported_and_pruned() {
    ...
    for code in ["E201", "W305", "U502"] {
        assert!(out.contains(code), "{} should be listed as resolved: {}", code, out);
    }
    assert!(out.contains("3 resolved"), "{}", out);

    let (code, out, _) = run(&[f2.to_str().unwrap(), "--write-baseline", base.to_str().unwrap()]);
    assert_eq!(code, 0, "{}", out);
    let b = gedlint::parse_baseline(&fs::read_to_string(&base).unwrap()).unwrap();
    assert!(b.entries.is_empty(), "the rewrite must prune resolved entries: {:?}", b.entries);
...
fn missing_baseline_file_is_a_clear_error() {
    ...
    let (code, out, err) = run(&[f.to_str().unwrap(), "--baseline", gone.to_str().unwrap(), "--no-color"]);
    assert_eq!(code, 2, "out: {} err: {}", out, err);
    assert!(err.contains("cannot read baseline") && err.contains(gone.to_str().unwrap()), "{}", err);
}
...
fn corrupt_baseline_file_is_a_clear_error() {
    ...
    let (code, out, err) = run(&[f.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--no-color"]);
    assert_eq!(code, 2, "out: {} err: {}", out, err);
    assert!(err.contains("baseline file:") && err.contains("expected"), "{}", err);
}
...
fn json_output_stays_full_and_exit_follows_the_ratchet() {
    ...
    let (code, out, _) = run(&[f.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--format", "json"]);
    assert_eq!(code, 0, "{}", out);
...
fn quiet_reports_the_ratchet_counts() {
    ...
    let base = write(&d, "g.baseline.json", "");
    let _ = run(&[f.to_str().unwrap(), "--write-baseline", base.to_str().unwrap()]);
    let (code, out, _) = run(&[f.to_str().unwrap(), "--baseline", base.to_str().unwrap(), "--quiet"]);
    assert_eq!(code, 0);
...
fn report(diags: Vec<Diag>) -> Report {
    Report { version: Version::V551, diags, lines: 0, individuals: 0, families: 0 }
}
...
fn counts_absorb_and_surplus_is_new() {
    ...
    let b = Baseline {
        entries: vec![BaselineEntry { code: "E201".to_string(), fingerprint: "famc @f#@ points nowhere".to_string(), count: 2 }],
    };
...
fn from_report_sorts_and_absorbs_shifted_line_references() {
    ...
    for i in 0..516 {
        diags.push(diag("U502", i * 3, "_UPD is not part of GEDCOM 5.5.1 or 7.0"));
    }
    for i in 0..171 {
        diags.push(diag("E001", i * 7 + 1, "malformed line (non-numeric level): orphan"));
    }
    ...
    assert_eq!(codes, ["E001", "E003", "U502"], "entries must be sorted by code");
    ...
    let shifted = report(vec![diag("E003", 52, "duplicate xref @F1@ (first at line 40)")]);
