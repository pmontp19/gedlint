...
use gedlint::wasm::{gedlint_alloc, gedlint_apply, gedlint_dealloc, gedlint_edits, gedlint_lint, gedlint_registry};
use gedlint::RULES;
...
fn with_bufs<T>(data: &[u8], cfg: &[u8], f: unsafe extern "C" fn(*const u8, usize, *const u8, usize) -> T) -> T {
...
fn lint_accepts_a_preset_config_and_reports_config_errors() {
    ...
    assert!(j.contains("W702"), "hygiene preset must reach the engine: {}", j);
...
fn registry_json_carries_every_rule_and_its_documentation() {
    ...
    for r in RULES {
        assert!(j.contains(&format!("\"code\":\"{}\"", r.code)), "{} missing", r.code);
    }
...
fn edits_json_matches_compute_edits() {
    ...
    assert!(j.contains(&format!("\"code\":\"E101\",\"start\":{},\"end\":{}", e.lines.0, e.lines.1)), "{}", j);
...
fn edits_and_apply_honour_the_config() {
    ...
    assert!(!def.contains("W601"), "the default config must not offer the opt-in repair: {}", def);
    assert!(on.contains("\"code\":\"W601\""), "the preset must reach the repair engine: {}", on);
    assert_eq!(on.matches("\"code\":").count(), def.matches("\"code\":").count() + 1);
    ...
    assert!(json.contains("Rovira, Font"), "the file must pass through unchanged");
    ...
    assert!(file.windows(b"1 NAME Maria /Rovira Font/".len()).any(|w| w == b"1 NAME Maria /Rovira Font/"),
...
fn apply_returns_summary_plus_file_bytes() {
    ...
    let n_str: String = json[bytes_at..].chars().take_while(|c| c.is_ascii_digit()).collect();
...
fn apply_a_subset_and_an_empty_selection() {
    ...
    let mask: Vec<u8> = all.iter().map(|e| if e.code == "E001" { 1 } else { 0 }).collect();
    let payload = take(apply_bufs(&data, &mask, b""));
    let n = u64::from_le_bytes(payload[..8].try_into().unwrap()) as usize;
    let json = String::from_utf8(payload[8..8 + n].to_vec()).unwrap();
    let file = &payload[8 + n..];
    ...
    assert!(!json.contains("\"applied\":[{\"code\":\"E101\""), "{}", json);
    assert!(file.windows(13).any(|w| w == b"2 CONT landed"), "{}", String::from_utf8_lossy(file));
    // The unselected split CONC survives untouched, bytes and all.
    let untouched = b"Jos\xC3\n2 CONC \xA9 /Oso/";
    assert!(file.windows(untouched.len()).any(|w| w == untouched), "{}", String::from_utf8_lossy(file));
...
fn apply_reports_postponed_overlaps() {
    ...
    let json = String::from_utf8(payload[8..8 + n].to_vec()).unwrap();
    assert!(json.contains("\"code\":\"E001\""), "{}", json);
    assert!(json.contains("\"postponed\":[{\"code\":\"style\""), "{}", json);
}
