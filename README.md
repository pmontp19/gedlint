...
- **`src/registry.rs`**: `RULES`, one `RuleMeta` per rule (code, name, ruleset, category, default severity, fixability, title, why, remedy). Single source of truth for `--explain`, and for the config validator, the generated docs and the web viewer's finding card as they land. `tests/registry.rs` fails the build if a code the engine emits has no entry, or an entry names a code the engine never emits.
